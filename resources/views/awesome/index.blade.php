  ...
  <script>
    function awesomeCalcApp() {
      return {
        ...
        calcList: [],
        async init() {
          // On load, fetch list of calculators
        ...
        async saveCalculator() {
          try {
            ...
            // If there's an ID, we might do an update, but let's keep it simple (create only).
            // For updates, you'd store the ID in calcForm, etc.
            ...
            if (!resp.ok) {
              console.error('Error saving calculator:', result);
              alert('Save failed. Check console.');
            } else {
              alert('Calculator saved!');
              await this.fetchAllCalculators();
              this.tab = 'list'; // switch to listing
            }
          } catch (err) {
            console.error('saveCalculator error:', err);
            alert('Save encountered an error. See console.');
          }
        },
        async loadCalculator(id) {
          try {
            let url = '/api/awesome/' + id;
            let resp = await fetch(url);
            let data = await resp.json();
            // Load into form:
            this.calcForm = {
